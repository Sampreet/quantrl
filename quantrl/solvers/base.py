...
class BaseIVPSolver(ABC):
    ...
    The inherited classes should contain the ``solver_methods`` attribute
    listing all the available methods of the corresponding solver.
    ...
    func: callable
        ...
        The first element of ``args`` contains the constant parameters,
        the second element contains the function for the controls
        and the third contains the delay function.
    solver_params: dict
        ...
        Currently supported options are:
            ...
            key                 value
            ================    ============================================
            method              (*str*) method used to solve the ODEs.
                                Refer to the documentation of the inherited
                                solvers. Default is ``'vode'`` from
                                :class:`quantrl.solvers.numpy.SciPyIVPSolver`.
            atol                (*float*) absolute tolerance of the integrator.
                                Default is ``1e-12``.
            rtol                (*float*) relative tolerance of the integrator.
                                Default is ``1e-9``.
            is_stiff            (*bool*) option to select whether the
                                integration is a stiff problem or
                                a non-stiff one. Default is ``False``.
    ...
    func_delay: callable
        ...
        This function is then internally replaced
        by the interpolated function for the subsequent steps.
    delay_interval: int
        Interval of the delay.

    ..note: In the presence of delay,
    the parameter ``'step_interval'``
    is overriden by the delay interval.
    """
    ...
    def __init__(
        ...
        # validate attributes
        assert self.func_delay is not None \
            if self.has_delay \
            else True, \
            "delay function cannot be ``None`` if parameter ``has_delay`` is ``True``"
        ...
        # validate params
        assert self.solver_params['method'] in self.solver_methods, \
            f"parameter ``method`` should be one of ``{self.solver_methods}``"
        assert isinstance(self.solver_params['step_interval'], int) \
            and self.solver_params['step_interval'] < self.shape_T[0], \
            "parameter ``step_interval`` should be an integer " \
                + "with a value less than the total number of steps"
