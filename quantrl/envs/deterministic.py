...
class LinearizedHOEnv(BaseGymEnv):
    ...
    def func(self,
        ...
        if self.num_corrs != 0:
            ...
            self.y_rates[2 * self.num_modes:] = np.add(np.add(np.matmul(self.A, corrs, out=self.matmul_0), np.matmul(corrs, self.A.transpose(), out=self.matmul_1), out=self.sum_0), self.D, out=self.sum_1).flatten()
...
class LinearizedHOVecEnv(BaseSB3Env):
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        for key in self.default_ode_solver_params:
            kwargs[key] = kwargs.get(key, self.default_ode_solver_params[key])
        # set matrices
        self.A = np.zeros((n_envs, *self.dim_corrs), dtype=np.float_)
        self.D = np.zeros((n_envs, *self.dim_corrs), dtype=np.float_)
        ...
        # initialize buffers
        if self.num_modes != 0:
            self.mode_rates_real = np.zeros((self.n_envs, 2 * self.num_modes), dtype=np.float_)
        if self.num_corrs != 0:
            self.matmul_0 = np.empty((self.n_envs, *self.dim_corrs), dtype=np.float_)
            self.matmul_1 = np.empty((self.n_envs, *self.dim_corrs), dtype=np.float_)
            self.sum_0 = np.empty((self.n_envs, *self.dim_corrs), dtype=np.float_)
            self.sum_1 = np.empty((self.n_envs, *self.dim_corrs), dtype=np.float_)
        self.y_rates = np.empty((self.n_envs, 2 * self.num_modes + self.num_corrs), dtype=np.float_)
    ...
    def _step(self):
        ...
        # solve coupled ODEs/DDEs using SciPyIVPSolver
            ...
            return self.solver.step_ivp(
                y0=self.Observations[-1].flatten(),
                T_step=self.T_step,
                params=self.actions
            ).reshape((self.T_step.shape[0], self.n_envs, 2 * self.num_modes + self.num_corrs))
    ...
    def func(self,
        ...
        # reshape
        y = np.reshape(y, (self.n_envs, 2 * self.num_modes + self.num_corrs))
        ...
        if self.num_corrs != 0:
            corrs = np.reshape(y[:, 2 * self.num_modes:], (self.n_envs, *self.dim_corrs))
            ...
            # get flattened correlation rates
            self.y_rates[:, 2 * self.num_modes:] = np.add(np.add(np.matmul(self.A, corrs, out=self.matmul_0), np.matmul(corrs, np.transpose(self.A, axes=(0, 2, 1)), out=self.matmul_1), out=self.sum_0), self.D, out=self.sum_1).reshape((self.n_envs, self.num_corrs))

        return self.y_rates.flatten()
