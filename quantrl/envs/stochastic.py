...
class LinearEnv(BaseGymEnv):
    """Class to interface stochastic linear
    environments using Wiener increments.

    Initializes ``A`` and ``is_A_constant``.
    The interfaced environment requires
    ``default_params`` dictionary defined
    before initializing the parent class.
    ...
    The ``func`` method requires ``get_A``
    for the Jacobian of the states and the
    ``get_noise_prefixes`` for the noise values.
    ...
    def __init__(
        ...
        super().__init__(
            ...
            dir_prefix=(dir_prefix \
                if dir_prefix != "data" \
                else ("data/" + self.name.lower()) + "/env") + "_" + "_".join(
    ...
    def get_A(
        ...
        -------
        A: Any
            Jacobian of the states with shape
            ``(n_observations, n_observations)``.
        """

        raise NotImplementedError

    def get_noise_prefixes(
            self,
            t_idx:int,
            args:tuple,
    ):
        ...
        -------
        noise_prefixes: Any
            Noise prefixes for each observation with shape
            ``(n_observations, )``.
        """
